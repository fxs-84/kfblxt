...
/** 根据 API URL 自动选 headers(Anthropic 用 x-api-key, OpenAI-compatible 用 Bearer) */
function buildRequestHeaders(apiKey: string, apiUrl: string): Record<string, string> {
...
export async function runAgent(
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(cfg.apiUrl, {
      method: "POST",
      headers: buildRequestHeaders(cfg.apiKey, cfg.apiUrl),
      body: JSON.stringify({
        model: cfg.model,
        max_tokens: 4000,
    ...
    if (!res.ok) {
      ...
      const hint = errText.slice(0, 200);
      throw new Error(`API ${res.status}${hint ? ": " + hint : ""}`);
    }
