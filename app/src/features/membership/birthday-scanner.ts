 ...
 * 找出当天过生日的患者,触发 patient.birthday 事件。
 *
 * 依赖:
 *  - patientRepository.findAll() — 取出所有患者
 *  - membershipBus.emit(...) — 投递到规则引擎
 *
 * 同一天内同一患者不会重复触发:规则引擎端的 cooldownDays 控制。
...
export function startBirthdayScanner(): () => void {
  ...
  void scanAndEmitBirthdays().then((n) => {
    if (n > 0) {
      console.info(`[membership] 🎂 ${n} 位患者今日过生日,已派发积分`);
    }
  });
  ...
  const scheduleNext = () => {
    ...
    midnightTimer = setTimeout(async () => {
      ...
      if (n > 0) console.info(`[membership] 🎂 每日扫描:${n} 位患者过生日`);
      // 切换到 24h 间隔
      dailyInterval = setInterval(async () => {
        const cnt = await scanAndEmitBirthdays();
        if (cnt > 0) console.info(`[membership] 🎂 每日扫描:${cnt} 位患者过生日`);
      }, ONE_DAY_MS);
    }, ms);
  };
  scheduleNext();
